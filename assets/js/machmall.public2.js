...
$.fn.alertTip= function (tipTxt,t) {
    console.log($('.pop-tip:visible')==true)
...
$.fn.cityMap=function (slid) {
    ...
    $.getJSON(slid.url, function (a) {
        ...
        arr.map(function (b) {
            ...
            $('.provi'+b.id).click(function () {
                ...
                $('.china-list').empty();
                $('.china-list').append('<div class="city-list"><div class="proviBox">'+$(this).text()+'<div></div>');
                       ...
                       var f=$(this).attr('class').substring(4,$(this).attr('class').length);
                        ...
                        arr.map(function (q) {
                            if(q<0){
                                ...
                                $('.china-list').append('<dl class="country-list"><dd class="cityBox">'+$(this).text()+'</dd></dl>');
    ...
    china.map(function (a) {
        /*for(var b in a){
            $('.city-box').css({display:'none'});
            $('.china-list ul').append('<li class="provi'+ a[b].id+'">'+  a[b].name+'</li>');

            $('.provi'+a[b].id).click(function () {
                $('.china-list').empty();

                $('.china-list').append('<div class="city-list"><div class="proviBox">'+$(this).text()+'<div></div>');
                for(var c in a[s].children){
                    console.log(a[s].children[c].name)
                    for(var d in c){
                        console.log(d)
                        $('.city-list').append('<div class="city'+a[b].children[c].id+'">'+ a[b].children[c].name+'</div>');
                    }
                    console.log(c);
       ...
       /* $('.city-box').css({display:'none'});
        $('.china-list ul').append('<li class="provi'+i+'">'+ p.name+'</li>');
        $('.provi'+i).click(function () {
            $('.china-list').empty();
            $('.china-list').append('<div class="city-list"><div class="proviBox">'+$(this).text()+'<div></div>');
            p.sub.map(function (s,j) {
                if(j==0) {
                    $('.city-list').append('');
                }else{
                    $('.city-list').append('<div class="city'+j+'">'+ s.name+'</div>');
                }

                $('.city'+j).click(function () {
                    if(!s.sub){
                        $('.china-list').empty();
                        mySwiper.slideTo(slid.slideNum, 500, false);//速度为0.5秒
                        th.val(p.name+' '+s.name);
                        return true;
                    }
                    $('.china-list').empty();
                    $('.china-list').append('<dl class="country-list"><dd class="cityBox">'+$(this).text()+'</dd></dl>');
                    s.sub.map(function (c,t) {
                        if(t==0) $('.city-list').append('');
                        else $('.country-list').append('<dd class="country'+t+'">'+ c.name+'</dd>');
                        $('.country'+t).click(function () {
                            $('.china-list').empty();
                            mySwiper.slideTo(slid.slideNum, 500, false);
                            console.log(slid.slideNum)
                            th.val(p.name+' '+s.name+' '+c.name);
